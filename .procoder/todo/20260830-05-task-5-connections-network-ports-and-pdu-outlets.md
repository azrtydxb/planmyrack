...
- [ ] Write the failing test `packages/core/test/links.test.ts`: Run `npm test -w @planmyrack/core` — expect FAIL with "does not provide an export named 'connect'".
- [ ] Implement `links.ts` and wire `pruneLinks` into `updateDevice`, `removeDevice` and `removeRack`. `updateDevice` clamps `ports`/`outlets` to the new type's maxima before pruning, so a type change to a portless type zeroes both counts.
