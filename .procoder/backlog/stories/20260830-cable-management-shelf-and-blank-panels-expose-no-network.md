...
- [x] Cable management, shelf and blank panels expose no network-port field in the inspector — `TestPortlessTypesHidePortField` — fails if a port control appears for a type whose maxPorts is 0.

## Evidence

- `TestPortlessTypesHidePortField`: no port field for hooks, brush, shelf or blank; switches get ports and PDUs get outlets.
