...
const BANNED = /^(react|react-dom|react-native|@react-native(?:-community)?\/[\w.-]+|expo(?:-[\w.-]+)?|expo\/[\w.-]+)$/
    ...
    for await (const file of sources(dir)) {
      ...
      for (const [, specifier] of text.matchAll(IMPORT)) {
        if (BANNED.test(specifier)) {
          hits.push({ file: relative(process.cwd(), file).split(sep).join('/'), module: specifier })
        }
      }
    }
  }
